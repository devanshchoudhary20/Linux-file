...
async function extractProductInfo(html, url) {
  ...
  // If there's no discounted price, check for the combined price
  if (!discountedPrice) {
    const combinedPrice = $('h4[data-testid="pdp-discounted-selling-price"]').text().trim();
    sellingPrice = combinedPrice;
    discountedPrice = combinedPrice;
  }

  const discountPercentage = $('.tag_tag__Y2gMh').text().trim();
...
async function processLinks() {
  ...
  for (const link of productLinks) {
    ...
    if (html) {
      const productInfo = await extractProductInfo(html, link);
      if (productInfo) {
        ...
        console.log(`Saved/Updated product: ${productInfo.title}`);
      }
    }
...
exports.processProducts = functions.https.onRequest(async (req, res) => {
  try {
    await processLinks();
